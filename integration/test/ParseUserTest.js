...
describe("Parse User", () => {
  ...
  it("can login users with installationId", async () => {
    ...
    expect(sessions[1].get("sessionToken")).toBe(
      installationUser.getSessionToken()
    );
  });
  ...
  it("can loginWith subclass static", async () => {
    ...
    await user.signUp();
    user = await CustomUser.logInWith(
  ...
  it("linked account can login with authData", async () => {
    ...
    Parse.User.enableUnsafeCurrentUser();
    const loggedIn = await Parse.User.logInWith(
      provider.getAuthType(),
      provider.getAuthData()
    );
  ...
  it("can encrypt user", async () => {
    ...
    const decryptedUser = crypto.decrypt(
      encryptedUser,
