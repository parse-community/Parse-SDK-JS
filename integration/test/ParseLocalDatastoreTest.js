...
function runTest(controller) {
  describe(`Parse Object Pinning (${controller.name})`, () => {
    ...
    it(`${controller.name} can pin (unsaved)`, async () => {
      ...
      assert.deepEqual(localDatastore[LDS_KEY(object)], [
        LDS_FULL_JSON(object),
      ]);
      await object.save();
    ...
    it(`${controller.name} can pin (recursive)`, async () => {
      ...
      assert.equal(
        localDatastore[DEFAULT_PIN].includes(LDS_KEY(grandchild)),
        true
      );
      ...
      assert.deepEqual(localDatastore[LDS_KEY(grandchild)], [
        grandchild._toFullJSON(),
      ]);
    });
    ...
    it(`${controller.name} can unPin on destroy`, async () => {
      ...
      assert.equal(
        localDatastore[PIN_PREFIX + "test_pin"].includes(LDS_KEY(obj1)),
        true
      );
      assert.equal(
        localDatastore[PIN_PREFIX + "test_pin"].includes(LDS_KEY(obj2)),
        true
      );
    ...
    it(`${controller.name} can unPin on destroyAll`, async () => {
      ...
      assert.equal(
        localDatastore[PIN_PREFIX + "test_pin"].includes(LDS_KEY(obj3)),
        true
      );
      assert(localDatastore[LDS_KEY(obj1)]);
      assert(localDatastore[LDS_KEY(obj2)]);
    ...
    it(`${controller.name} can unPin and save reference with children`, async () => {
      ...
      assert.equal(
        localDatastore[PIN_PREFIX + "test_pin"].includes(LDS_KEY(child)),
        true
      );
      assert(localDatastore[LDS_KEY(obj1)]);
      assert(localDatastore[LDS_KEY(obj2)]);
    ...
    it("fetchAll updates LocalDatastore", async () => {
      ...
      const fetchedItems = await Parse.Object.fetchAll([
        item1Again,
        item2Again,
      ]);
  ...
  describe(`Parse Query Pinning (${controller.name})`, () => {
    ...
    it(`${controller.name} can test objectId in containedIn queries`, async () => {
      const numbers = await new Parse.Query("BoxedNumber")
        .ascending("number")
        .find();
    ...
    fit(`${controller.name} supports objects with length`, async () => {
      const obj = new TestObject();
      obj.set("length", 5);
    ...
    it(`${controller.name} can query from pin subclass`, async () => {
      ...
      let localDatastore = await Parse.LocalDatastore._getAllContents();
      expect(localDatastore[LDS_KEY(testClassB)][0].classA).toEqual(
        testClassA.toOfflinePointer()
      );
      ...
      localDatastore = await Parse.LocalDatastore._getAllContents();
      expect(localDatastore[LDS_KEY(testClassB)][0].classA.objectId).toEqual(
        testClassA.id
      );
    });
  });
