...
class ParseObject {
   ...
   * @param {String|Array<String>|...String} [keys] - specify which fields to revert
   */
    ...
    if(keys.length) {
      ...
      keys.forEach((key) => {
        if (Array.isArray(key)) {
          _keysToRevert = _keysToRevert.concat(key);
        } else {
          _keysToRevert.push(key);
        }
      });
    }
    this._clearPendingOps(_keysToRevert);
  }
