...
describe("RESTController", () => {
  ...
  it("resolves with the result of the AJAX request", (done) => {
    RESTController._setXHR(
      mockXHR([{ status: 200, response: { success: true } }])
    );
  ...
  it("idempotency - handle requestId on network retries", (done) => {
    ...
    RESTController.ajax("POST", "users", {}).then(
      ({ response, status, xhr }) => {
        ...
        expect(
          requestIdHeaders.every(
            (header) => header[1] === requestIdHeaders[0][1]
          )
        ).toBeTruthy();
  ...
  it("includes the status code when requested", (done) => {
    ...
    RESTController.request("POST", "users", {}, { returnStatus: true }).then(
      (response) => {
  ...
  it("can handle wechat ajax", async () => {
    ...
    expect(xhr.getAllResponseHeaders().includes("X-Parse-Session-Token")).toBe(
      true
    );
