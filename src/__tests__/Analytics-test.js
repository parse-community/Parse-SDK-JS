...
describe("Analytics", () => {
  ...
  it("throws when no event name is provided", () => {
    expect(Analytics.track).toThrow(
      "A name for the custom event must be provided"
    );

    expect(Analytics.track.bind(null, "")).toThrow(
      "A name for the custom event must be provided"
    );
  });
  ...
  it("trims whitespace from event names", () => {
    ...
    expect(CoreManager.getAnalyticsController().track.mock.calls[1]).toEqual([
      "after",
      {},
    ]);

    Analytics.track("  both  ", {});
    expect(CoreManager.getAnalyticsController().track.mock.calls[2]).toEqual([
      "both",
      {},
    ]);
  });
