...
describe("ParseFile", () => {
  ...
  it("can extract data type from base64 with a charset param", () => {
    const file = new ParseFile("parse.kml", {
      base64:
        "data:application/vnd.3gpp.pic-bw-var;charset=utf-8;base64,ParseA==",
    });
    expect(file._source.base64).toBe("ParseA==");
  ...
  it("returns secure url when specified", () => {
    ...
    return file.save().then(function (result) {
      ...
      expect(result.url({ forceSecure: true })).toBe(
        "https://files.parsetfss.com/a/parse.txt"
      );
    });
  });
  ...
  it("can construct a file from a JSON object", () => {
    ...
    expect(ParseFile.fromJSON.bind(null, {})).toThrow(
  ...
  it("reports progress during save when source is a File", () => {
    const file = new ParseFile(
      "progress.txt",
      new File(["Parse"], "progress.txt")
    );
  ...
  it("should save file with metadata and tag options", async () => {
    ...
    const file = new ParseFile(
      "donald_duck.txt",
      new File(["Parse"], "donald_duck.txt")
    );
...
describe("FileController", () => {
  ...
  it("saveUri with uri type", async () => {
    ...
    const spy = jest
      .spyOn(defaultController, "download")
  ...
  it("download with base64 http", async () => {
    ...
    const data = await defaultController.download(
  ...
  it("download with base64 http abort", async () => {
    ...
    defaultController
      .download("http://example.com/image.png", options)
      .then((data) => {
        expect(data).toEqual({});
      });
  ...
  it("download with ajax abort", async () => {
    ...
    defaultController
      .download("https://example.com/image.png", options)
      .then((data) => {
        expect(data).toEqual({});
      });
    _requestTask.abort();
  ...
  it("download with ajax error", async () => {
    ...
    try {
      await defaultController.download(
        "https://example.com/image.png",
        options
      );
    } catch (e) {
  ...
  it("saves files via ajax with sessionToken option", () => {
    ...
    return file
      .save({ sessionToken: "testing_sessionToken" })
      .then(function (f) {
        expect(f).toBe(file);
        expect(f.name()).toBe("parse.txt");
        expect(f.url()).toBe("https://files.parsetfss.com/a/parse.txt");
  ...
  it("should handle delete file error", async () => {
    ...
    const ajax = jest
      ...
      .mockResolvedValueOnce(
        Promise.reject(new ParseError(403, "Cannot delete file."))
      );
