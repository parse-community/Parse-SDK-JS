...
describe("LocalDatastore", () => {
  ...
  it("_handleUnPinAllWithName default pin", async () => {
    ...
    ).toHaveBeenCalledWith(DEFAULT_PIN, [KEY2]);
  });
  ...
  it("_handleUnPinAllWithName specific pin", async () => {
    ...
    expect(
      mockLocalStorageController.pinWithName
    ).toHaveBeenCalledWith(PIN_PREFIX + "test_pin", [KEY2]);
  });
  ...
  it("_handleUnPinAllWithName remove if exist", async () => {
    ...
    expect(
      mockLocalStorageController.pinWithName
    ).toHaveBeenCalledWith(PIN_PREFIX + "test_pin", [KEY2, KEY3]);
  });
  ...
  it("_updateObjectIfPinned if pinned", async () => {
    mockLocalStorageController.fromPinWithName.mockImplementationOnce(() => [
      item1,
    ]);
    ...
    expect(
      mockLocalStorageController.fromPinWithName.mock.results[0].value
    ).toEqual([item1]);
  ...
  it("_serializeObjectsFromPinName no objects", async () => {
    ...
    const results = await LocalDatastore._serializeObjectsFromPinName(
      DEFAULT_PIN
    );
