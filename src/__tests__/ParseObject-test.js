...
describe("ParseObject pin", () => {
  ...
  it("can pin to default", async () => {
    ...
    expect(
      mockLocalDatastore._handlePinAllWithName
    ).toHaveBeenCalledWith(DEFAULT_PIN, [object]);
  });
  ...
  it("can unPin to default", async () => {
    ...
    expect(
      mockLocalDatastore._handleUnPinAllWithName
    ).toHaveBeenCalledWith(DEFAULT_PIN, [object]);
  });
  ...
  it("can pin to specific pin", async () => {
    ...
    expect(
      mockLocalDatastore._handlePinAllWithName
    ).toHaveBeenCalledWith("test_pin", [object]);
  });
  ...
  it("can fetchFromLocalDatastore", async () => {
    ...
    mockLocalDatastore._serializeObject.mockImplementationOnce(() =>
      object._toFullJSON()
    );
  ...
  it("can unPinAllObjects", async () => {
    ...
    expect(mockLocalDatastore.unPinWithName.mock.calls[0]).toEqual([
      DEFAULT_PIN,
    ]);
  });
