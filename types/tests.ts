...
function test_errors() {
    try {
        throw new Parse.Error(Parse.Error.OTHER_CAUSE, "sdfds");
    } catch (error) {
        if (error.code !== 1) {
...
function test_each() {
    new Parse.Query(GameScore).each(game => {
        // $ExpectType Game
        game;
    });
}
...
function test_relation() {
    const game1 = new GameScore();
    const game2 = new GameScore();
...
async function test_cloud_functions() {
    ...
    Parse.Cloud.afterSave("MyCustomClass", (request: Parse.Cloud.TriggerRequest) => {
        if (!request.context) {
    ...
    Parse.Cloud.beforeDelete("MyCustomClass", (request: Parse.Cloud.TriggerRequest) => {
        // result
    });

    Parse.Cloud.beforeDelete("MyCustomClass", async (request: Parse.Cloud.TriggerRequest) => {
        // result
    });
